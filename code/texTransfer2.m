function [ Output ] = texTransfer2(OrigTex, TargetImage, BWidth, O)
...
%             UpBlock = Output(i-O:i-1,j-O:min(j-O-1+BWidth,OutN),:);
            ...
            if i > BWidth
                if j > BWidth
                      Block = texCut2(CorrOrig, OrigTex,CBlock,patch,Both,alpha,O,k,Output);
                else
                     Block = texCut2(CorrOrig, OrigTex,CBlock,patch,Up,alpha,O,k,Output);                
                end
                Output(i-O:min(i-O-1+BWidth,OutM),j-O:min(j-O-1+BWidth,OutN),:) = Block;
            end
            if (i==O+1) && (j>BWidth)
                Block = texCut2(CorrOrig,OrigTex,CBlock,patch,Left,alpha,O,k,Output); 
                Output(i-O:min(i-O-1+BWidth,OutM),j-O:min(j-O-1+BWidth,OutN),:) = Block;
            end
